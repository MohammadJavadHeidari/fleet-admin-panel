...
export function OtpVerifyView() {
  ...
  const employeePhoneNumber = searchParams.get('phoneNumber') || '';
  ...
  const onSubmit = async (data: OtpVerifyFormValues) => {
    try {
      await verifyOtp?.(employeePhoneNumber , data.code);

      window.location.href = paths.employee.dashboard.root;
    } catch (error) {
      console.error(error);
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      ...
      > 
        ...
        <Typography variant="body1">
          کد تایید را به شماره {employeePhoneNumber} ارسال کردیم.
        </Typography>

        <Typography variant="body1" sx={{ mt: 1, gap: 1, display: 'flex', alignItems: 'center' }}>
          شماره موبایل اشتباه است؟ 
          <RouterLink href={`${paths.employee.auth.requestOtp}`}>
